...
void DisplayManager::init() { 
    display.begin(); 
    display.setFont(u8g2_font_6x10_tf); // Читаемый моноширинный шрифт
...
void DisplayManager::handleInput(InputEvent evt) {
    ...
    // Мы разрешаем менять индексы меню ТОЛЬКО если пользователь ВИДИТ это меню.
    // Если идет атака, кнопки UP/DOWN игнорируются (кроме BACK, который обрабатывается в System).
    ...
    if (!isMenuVisible && !isListVisible && !isSubMenuVisible) {
        return; // Блокировка ввода для UI
    }
...
void DisplayManager::drawSpectrum() { 
    ...
    for(int x=0; x<128; x++) {
        ...
        if(val > 0) {
            int h = val / 2; if (h > 50) h = 50; 
            int yTop = 56 - h; if (yTop < 12) yTop = 12; 
...
void DisplayManager::drawBleMenu() {
    ...
    for(int i=0; i<3; i++) { 
        int y = 34 + (i*12); 
        if(i == _submenuIndex) { display.drawStr(0, y, ">"); display.drawStr(10, y, items[i]); } 
        else display.drawStr(10, y, items[i]); 
    }
}

void DisplayManager::drawNrfMenu() { 
    char buf[32]; snprintf(buf, 32, "Jam Channel: %d", _submenuIndex + 1); 
    display.drawStr(10, 40, buf); display.drawStr(10, 55, "< Select >");
}

void DisplayManager::drawPopup(const char* msg) { 
    display.setDrawColor(0); display.drawBox(10, 20, 108, 30); display.setDrawColor(1);
    display.drawFrame(10, 20, 108, 30); display.drawBox(10, 20, 108, 12); 
    display.setDrawColor(0); display.drawStr(45, 30, "INFO");
    display.setDrawColor(1); display.drawStr(15, 45, msg);
}
...
void DisplayManager::drawAdminScreen() {
    ...
    if (_currentStatus.state == SystemState::WEB_CLIENT_CONNECTED) display.drawStr(0, 60, "Client: Connected");
    else display.drawStr(0, 60, "Client: Waiting...");
    display.setFont(u8g2_font_6x10_tf);
}

void DisplayManager::updateStatus(const StatusMessage& msg) { _currentStatus = msg; _isDirty = true; }
void DisplayManager::showSplashScreen() { display.clearBuffer(); display.setFont(u8g2_font_ncenB10_tr); display.drawStr(15,35,"nRF Ghost"); display.setFont(u8g2_font_6x10_tf); display.drawStr(40,50,"v5.5"); display.sendBuffer(); _isDirty=true; }
