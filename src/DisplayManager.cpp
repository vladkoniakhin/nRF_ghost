...
void DisplayManager::handleInput(InputEvent evt) {
    ...
    if (!isMenuVisible && !isListVisible && !isSubMenuVisible) return;
...
void DisplayManager::render() {
    // FIX v6.4: ADAPTIVE FPS (SPI STARVATION SOLVER)
    static uint32_t lastRender = 0;
    
    // Если идет запись на SD (например, дамп WiFi или SubGHz),
    // мы обновляем экран ОЧЕНЬ редко (раз в секунду), чтобы не занимать SPI шину.
    // Если простой - 30 FPS (33мс).
    int interval = SdManager::getInstance().isCapturing() ? 1000 : 33; 
    ...
    switch (_currentStatus.state) {
        ...
        case SystemState::SCAN_EMPTY: drawPopup("No Networks Found"); break;
        ...
        case SystemState::WEB_CLIENT_CONNECTED: drawAdminScreen(); break;
...
void DisplayManager::drawStatusBar() {
    ...
    if (!_batteryInit) { _batteryFilterAccum = raw * 16; _batteryInit = true; }
...
void DisplayManager::drawMenu() {
    ...
    for (int i = 0; i < VISIBLE_ITEMS; i++) {
        ...
        if (idx == _menuIndex) {
            display.setDrawColor(1); display.drawBox(0, y, 128, ROW_HEIGHT); display.setDrawColor(0); 
...
void DisplayManager::drawTargetList() {
    if (_scanResults.empty()) { display.drawStr(10, 30, "Empty List"); return; }
    int visible = 4;
    int startIdx = 0;
    ...
    for (int i = 0; i < visible; i++) {
        ...
        int y = 14 + (i * 12);
        char buf[64]; char ssidSafe[22];
        int len = strlen(_scanResults[idx].ssid);
        if (len > 18) { strncpy(ssidSafe, _scanResults[idx].ssid, 15); ssidSafe[15] = 0; strcat(ssidSafe, "..."); } 
        else { strcpy(ssidSafe, _scanResults[idx].ssid); }
        snprintf(buf, sizeof(buf), "%s %d", ssidSafe, _scanResults[idx].rssi);
        if (idx == _targetIndex) { display.drawStr(0, y+9, ">"); display.drawStr(10, y+9, buf); } 
        else { display.drawStr(10, y+9, buf); }
    }
}

void DisplayManager::drawSpectrum() { 
    display.setFont(u8g2_font_4x6_tf); display.drawStr(0, 64, "Low"); display.drawStr(100, 64, "High");
    for(int x=0; x<128; x++) { uint8_t val = _currentStatus.spectrum[x]; if(val > 0) { int h = val / 2; if (h > 50) h = 50; int yTop = 56 - h; if (yTop < 12) yTop = 12; display.drawLine(x, 56, x, yTop); } }
    display.setFont(u8g2_font_6x10_tf);
}

void DisplayManager::drawAttackDetails() { 
    display.drawStr(0, 30, _currentStatus.logMsg); 
    if(_currentStatus.rollingCodeDetected) { display.setFont(u8g2_font_open_iconic_check_2x_t); display.drawGlyph(56, 55, 0x42); display.setFont(u8g2_font_6x10_tf); display.drawStr(20, 60, "ROLLING CODE"); }
    if(_currentStatus.handshakeCaptured) display.drawStr(20, 60, "HANDSHAKE!"); 
}

void DisplayManager::drawBleMenu() {
    const char* items[] = {"iOS (AirPods)", "Android (Fast)", "Win (Swift)"}; 
    display.drawStr(0, 22, "Spoof Target:");
    for(int i=0; i<3; i++) { int y = 34 + (i*12); if(i == _submenuIndex) { display.drawStr(0, y, ">"); display.drawStr(10, y, items[i]); } else display.drawStr(10, y, items[i]); }
}

void DisplayManager::drawNrfMenu() { 
    char buf[32]; snprintf(buf, 32, "Jam Channel: %d", _submenuIndex + 1); display.drawStr(10, 40, buf); display.drawStr(10, 55, "< Select >"); 
}

void DisplayManager::drawPopup(const char* msg) { 
    display.setDrawColor(0); display.drawBox(10, 20, 108, 30); display.setDrawColor(1); display.drawFrame(10, 20, 108, 30); display.drawBox(10, 20, 108, 12); display.setDrawColor(0); display.drawStr(45, 30, "INFO"); display.setDrawColor(1); display.drawStr(15, 45, msg); 
}

void DisplayManager::drawAdminScreen() {
    display.setFont(u8g2_font_5x8_tf); display.drawStr(0, 25, "SSID: nRF_Admin"); display.drawStr(0, 35, "IP:   192.168.4.1"); display.drawStr(0, 50, "Status: Active");
    if (_currentStatus.state == SystemState::WEB_CLIENT_CONNECTED) display.drawStr(0, 60, "Client: Connected"); else display.drawStr(0, 60, "Client: Waiting..."); display.setFont(u8g2_font_6x10_tf);
}

void DisplayManager::updateStatus(const StatusMessage& msg) { _currentStatus = msg; _isDirty = true; }
void DisplayManager::showSplashScreen() { display.clearBuffer(); display.setFont(u8g2_font_ncenB10_tr); display.drawStr(15,35,"nRF Ghost"); display.setFont(u8g2_font_6x10_tf); display.drawStr(40,50,"v6.4"); display.sendBuffer(); _isDirty=true; }
